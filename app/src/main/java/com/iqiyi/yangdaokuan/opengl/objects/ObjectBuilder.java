...
public class ObjectBuilder {
    ...
    static GeneratedData createMallet(Geometry.Point center, float radius, float height, int numPoints) {
        ...
        final float handHeight = height * 0.75f;
        final float handRadius = radius / 3f;
        Geometry.Circle handleCircle = new Geometry.Circle(center.translateY(height * 0.5f), handRadius);
        Geometry.Cylinder handleCylinder = new Geometry.Cylinder(handleCircle.center.translateY(-handHeight / 2f), handRadius, handHeight);
    ...
    private void appendCircle(Geometry.Circle circle, int numPoints) {
        ...
        drawList.add(new DrawCommand() {
            @Override
            public void draw() {
                glDrawArrays(GL_TRIANGLE_FAN, starVertex, numVertices);
            }
        });
    }

    private void appendOpenCylinder(Geometry.Cylinder cyclinder, int numPoints) {
        final int startVertex = offset / FLOATS_PER_VERTEX;
        final int numVertices = sizeOfOpenCylinderInVertices(numPoints);
        final float yStart = cyclinder.center.y - (cyclinder.height / 2f);
        final float yEnd = cyclinder.center.y + (cyclinder.height / 2f);
        for (int i = 0; i <= numPoints; i++) {
            float angleInRadians = ((float) i / (float) numPoints) * ((float) Math.PI * 2f);
            float xPosition = cyclinder.center.x + cyclinder.radius * (float) Math.cos(angleInRadians);
            float zPosition = cyclinder.center.z + cyclinder.radius * (float) Math.sin(angleInRadians);
